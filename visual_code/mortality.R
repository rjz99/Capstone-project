...
ALIVE_AT_3_YEAR.0.01 <- res[res$padj<0.05,] 
...
rld.mat <- assay(rld)
diff_gene.ALIVE_AT_3_YEAR <- subset(ALIVE_AT_3_YEAR.0.01, padj < 0.05 & abs(log2FoldChange) > 1 )
#summary(diff_gene.ALIVE_AT_3_YEAR)
...
sig.genes <- rownames(diff_gene.ALIVE_AT_3_YEAR[order(diff_gene.ALIVE_AT_3_YEAR$padj)[1:40],])
...
plot.dat.Z <- t(apply(plot.dat,1,cal_Z_stat))
plot.dat.Z <- plot.dat.Z[,rownames(df)]
plot(density(plot.dat.Z[1,]))
plot.dat.log2.Z <- plot.dat.log2.Z[,rownames(df)]
pheatmap::pheatmap(plot.dat.log2.Z,show_rownames = F,show_colnames = F, scale="row",cluster_cols = F,cluster_rows = F)

pheatmap(plot.dat.log2.Z,show_rownames = T,show_colnames = F,annotation_col = df,cluster_cols = F,cluster_rows = F)
pheatmap(plot.dat.Z,show_rownames = F,show_colnames = F,annotation_col = df,cluster_cols = F,cluster_rows = F)
pheatmap(plot.dat.log2.Z)



diff_data.ALIVE_AT_3_YEAR <- as.matrix(rld.mat[rownames(diff_gene.ALIVE_AT_3_YEAR[order(diff_gene.ALIVE_AT_3_YEAR$padj)[1:40],]), ]) 
...
Z.ALIVE_AT_3_YEAR <- apply(diff_data.ALIVE_AT_3_YEAR,1,cal_Z_stat)
Z.ALIVE_AT_3_YEAR <- t(Z.ALIVE_AT_3_YEAR)
df <- as.data.frame(colData(ddm)[,c("condition","gender")])
df <- df[order(df$condition),]
diff_data.ALIVE_AT_3_YEAR.plot <- diff_data.ALIVE_AT_3_YEAR[,rownames(df)]
Z.ALIVE_AT_3_YEAR <- Z.ALIVE_AT_3_YEAR[,rownames(df)]
pheatmap::pheatmap(Z.ALIVE_AT_3_YEAR,show_rownames = F,show_colnames = F,annotation_col = df,cluster_cols = F,cluster_rows = F)
