...
subject_survey_sub<-subject_summary[,c("idno","echo_rvdia_ge5","AGE","GENDER","bmi","ETIOLOGY")] 
subject_survey_sub <- na.omit(subject_survey_sub)
...
coldata <- data.frame(row.names = data1$RNAseq_ID, condition=data1$echo_rvdia_ge5,age=data1$AGE,gender=as.factor(data1$GENDER),bmi=data1$bmi,ETIOLOGY=data1$ETIOLOGY) 
## DESeq2 object
...
write.table(normalized_dat, file="E:/capstone/data/normalized_dat_echo_rvdia_ge5.txt", sep="\t", quote=F,row.names = F) 
...
echo_rvdia_ge5.0.05 <- res[res$padj<0.05,] ## 74 
summary(echo_rvdia_ge5.0.05)
...
diff_gene.echo_rvdia_ge5 <- subset(echo_rvdia_ge5.0.05, padj < 0.05 & abs(log2FoldChange) > 1)
summary(diff_gene.echo_rvdia_ge5)
...
sig.genes <- rownames(diff_gene.echo_rvdia_ge5[order(diff_gene.echo_rvdia_ge5$padj),])
plot.dat <- normalized_counts[sig.genes,] 
plot.dat.log2 <- log2(normalized_counts[sig.genes,]+1)
plot.dat.log2.Z <- t(apply(plot.dat.log2,1,cal_Z_stat))
plot.dat.Z <- plot.dat.Z[,rownames(df)]
plot(density(plot.dat.log2.Z[1,]))
plot.dat.log2.Z <- plot.dat.log2.Z[,rownames(df)]
#pheatmap::pheatmap(plot.dat.log2.Z,show_rownames = T,show_colnames = F, scale="row",cluster_cols = F,cluster_rows = F)


# add group annotation
anno<-data.frame(col.names=rownames(df),RV_dialation=df$condition)
anno$RV_dialation <- ifelse(1,"Dilated","Non-dilated")

# define the colours
library(RColorBrewer)
newCols <- colorRampPalette(grDevices::rainbow(length(unique(anno$RV_dialation))))
annoCol <- newCols(length(unique(anno$RV_dialation)))
names(annoCol) <- unique(anno$RV_dialation)
annoCol <- list(category = annoCol)

annoCol<-list(RV_dialation=c("Non-dilated"="blue", "Dilated"="red"))


pheatmap(plot.dat.log2.Z,show_rownames = T,show_colnames = F,
         annotation_col = anno,annotation_colors = annoCol,
         cluster_cols = F,cluster_rows = F)

pheatmap(plot.dat.log2.Z)



diff_data.echo_rvdia_ge5 <- as.matrix(rld.mat[rownames(diff_gene.echo_rvdia_ge5[order(diff_gene.echo_rvdia_ge5$padj),]), ]) 

## compute Z-statistic
cal_Z_stat <- function(x){
...
Z.echo_rvdia_ge5 <- apply(diff_data.echo_rvdia_ge5,1,cal_Z_stat)
Z.echo_rvdia_ge5 <- t(Z.echo_rvdia_ge5)
df <- as.data.frame(colData(ddm)[,c("condition","gender")])
df <- df[order(df$condition),]
df$condition <- ifelse(df$condition,1,0)
df$condition <- as.factor(df$condition)
diff_data.echo_rvdia_ge5.plot <- diff_data.echo_rvdia_ge5[,rownames(df)]
Z.echo_rvdia_ge5 <- Z.echo_rvdia_ge5[,rownames(df)]
pheatmap::pheatmap(Z.echo_rvdia_ge5,show_rownames = F,show_colnames = F,annotation_col = df,cluster_cols = F,cluster_rows = F)
pheatmap::pheatmap(diff_data.ALIVE_AT_3_YEAR.plot,show_rownames = F,show_colnames = F,annotation_col = df, scale="row",cluster_cols = F,cluster_rows = F)
